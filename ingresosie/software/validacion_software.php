...
$clave = $_POST['clave'];
$carrera_requerida = 'Software'; // Asegúrate de que la carrera requerida es correcta
...
if ($resultado->num_rows > 0) {
    ...
    if ($usuario['carrera'] === $carrera_requerida) {
        ...
        header("Location: soft/perfil.php");
        exit();
    } else {
        // Usuario válido pero no pertenece a esta carrera
        echo "<script>alert('Acceso denegado: No perteneces a la carrera de Software.'); window.location.href='sieingsoft.html';</script>";
        exit();
    }
} else {
